...
namespace WPF_Bestelbons
{
    public static class TextBlockHighlighter
    {
        ...
        private static void SelectText(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            string highlightText = GetSelection(d);
            if (string.IsNullOrEmpty(highlightText)) return;
            highlightText = highlightText.ToLower();
            string[] substrings = highlightText.Split('+'); ///////////////
            foreach (var str in substrings)
            {
                if (String.IsNullOrEmpty(str))
                {
                    substrings = substrings.Where(w => w != substrings.Last()).ToArray();
                }
            }
            ...
            foreach (var str in substrings)
            {
                int ind = text.IndexOf(str, StringComparison.InvariantCultureIgnoreCase);
                if (ind < 0) return;
                if (ind < firstindex)
                {
                    index = ind;
            ...
            while (true)
            {
                txtBlock.Inlines.AddRange(new Inline[] {
                    ...
                    new Run(text.Substring(index, firstsubstring.Length)) {Background = selectionColor,
                        Foreground = forecolor}
                });

                text = text.Substring(index + firstsubstring.Length);
                firstindex = 100;
                index = -1;
                foreach (var str in substrings)
                {
